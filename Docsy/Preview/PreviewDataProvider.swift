...
class PreviewDataProvider: BundleRepositoryProvider {
    static var bundle: PreviewDataProvider = try! PreviewDataProvider(rootPath: Bundle.main.resourcePath!)
...
extension PreviewDataProvider {
    ...
    private static func buildTree(root: URL, fileManager: FileManager = .default) throws -> FSNode {
        ...
        for url in childURLs {
            if FileManager.default.directoryExists(atPath: url.path) {
                try children.append(buildTree(root: url, fileManager: fileManager))
            } else {
    ...
    func findBundles(
        ...
        try findBundles(in: fileSystem, limit: limit, where: condition)
    }
    ...
    private func createBundle(
        ...
        let baseURL = URL(
            string: String(directory.url.path().trimmingPrefix(fileSystem.url.path()))
        )!
...
public extension FSNodeProtocol {
